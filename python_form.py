...
# sns.heatmap(E_Matrix_Reshaped, annot=True, fmt=".2f", cmap="viridis",
#             cbar_kws={'label': 'Probabilitate E(I, J)'}, linewidths=.5)

# plt.yticks(np.arange(12) + 0.5, [f'Clasa {i+1}' for i in range(12)], rotation=0)
# plt.xticks(np.arange(7) + 0.5, [f'Niv. {j+1}' for j in range(7)])

# plt.title('Harta Probabilitatilor Timbru vs. Nivel Densitate (Matricea E)')
# plt.ylabel('Clasa de Timbru (I)')
# plt.xlabel('Nivel de Densitate (J)')

# plt.tight_layout()
# plt.savefig('matrice_E_heatmap_final.png')
...
class FreeStochasticMusic:
    def __init__(self, TETA, Z1, Z2, E, KTR, KTE, V3, DELTA, VITLIM):
        self.TETA = TETA # distributie pitch
        self.Z1 = Z1
        self.Z2 = Z2 # parametri durata
        self.E = E # matricea timbru/densitate
        self.KTR = KTR # numar clase timbru
        self.KTE = KTE # numar nivele densitate
        self.V3 = V3 # coeficient Poisson
        self.DELTA = DELTA # durata minima
        self.VITLIM = VITLIM # durata maxima

        self.time = 0.0
        self.events = []
        self.time_window = 2.0

    # Densitate (alegere uniforma, controleaza densitatea evenimentelor, cat de des apar sunetele, cat de dispersate in pitch)
    def choose_density_level(self):
        return random.randint(0, self.KTE - 1)

    # Clasa timbru (matricea E)
    def choose_timbre_class(self, density_level): # rearanjare orchestrala, normalizare de probabilitati folosind matricea E si desnitatea aleasa
    ...
    def generate_duration(self): # generare durata folosind distributia exponentiala si incadrare in limite
    ...
    def generate_pitch(self, density):
        ...
        delta_index = np.random.choice(len(self.TETA)) # pitch cloud - distributie statistica
    ...
    def generate_event(self):
        ...
        X = np.random.rand()
        max_interval = 0.5
        inter_time = min(-np.log(X) / (self.V3 * np.exp(density)), max_interval)  # DA = V3*exp(U), Poisson timing
